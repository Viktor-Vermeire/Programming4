...
void dae::CSteamAchievements::onNotify(GameObject& entity, Event event)
{
	switch (event) {
	case (SCORE):
		auto playerComp = entity.GetComponent<dae::PlayerComponent>();
		if (playerComp->GetScore() == 500 && this) {
			if (this) {
				this->SetAchievement("ACH_WIN_ONE_GAME");
			}
		}
	}
}
